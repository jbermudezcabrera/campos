...
class SQLAlchemySource(FieldSource):
    ...
    that fields generated by the following code snippets are the same::
        ...
        # using an object
        user = User(id=1, name='Pepe', fullname='Pepe Sanchez', password='ok')
    ...
    def create_fields(self, attributes):
        from sqlalchemy import Boolean, Integer, Float, String, Text, Date, Enum, Time, DateTime
        ...
        for attr, text_column in filtered.items():
            ...
            if ctype_class == Boolean:
                ...
                kwargs = {} if ctype.precision is None else {'precision': ctype.precision}
                field = from_float(attr, text, 0, **kwargs)

            elif ctype_class == String:
                kwargs = {} if ctype.length is None else {'max_length': ctype.length}
